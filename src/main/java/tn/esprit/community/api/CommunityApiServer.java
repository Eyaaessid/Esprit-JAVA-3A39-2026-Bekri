package tn.esprit.community.api;
...
public class CommunityApiServer {
    ...
    public void start(CommunityContext appState, int port) throws IOException {
        if (server != null) {
            return;
        }
    ...
    private void handlePosts(HttpExchange exchange, CommunityContext appState) throws IOException {
    ...
    private void handleComments(HttpExchange exchange, CommunityContext appState) throws IOException {
        if (!"GET".equalsIgnoreCase(exchange.getRequestMethod())) {
            send(exchange, 405, "{\"error\":\"Method not allowed\"}");
            return;
        }
    ...
    private void handleDashboard(HttpExchange exchange, CommunityContext appState) throws IOException {
        if (!"GET".equalsIgnoreCase(exchange.getRequestMethod())) {
            send(exchange, 405, "{\"error\":\"Method not allowed\"}");
            return;
        }
