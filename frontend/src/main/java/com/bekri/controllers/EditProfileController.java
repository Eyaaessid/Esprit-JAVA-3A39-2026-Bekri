...
public class EditProfileController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Platform.runLater(() -> {
            UtilisateurResponse cached = SessionManager.getInstance().getCurrentUser();
            if (cached != null) {
        ...
        new Thread(() -> {
            try {
                UtilisateurResponse fresh = ApiClient.getMe();
    ...
    private void applyUserToForm(UtilisateurResponse user) {
        SessionManager.getInstance().setCurrentUser(user);
        origNom = nvl(user.getNom());
        origPrenom = nvl(user.getPrenom());
        origEmail = nvl(user.getEmail());
        origTel = nvl(user.getTelephone());
        origDateStr = user.getDateNaissance() != null && user.getDateNaissance().length() >= 10
                ? user.getDateNaissance().substring(0, 10) : null;
        origAvatar = user.getAvatar();
        ...
        if (origDateStr != null) {
            try {
                dateNaissancePicker.setValue(LocalDate.parse(origDateStr));
    ...
    private void refreshAvatarDisplay(UtilisateurResponse user) {
        if (avatarImageView == null) {
            return;
        }
        String url = ApiClient.absoluteUrl(user.getAvatar());
    ...
    private void showInitialsFallback(UtilisateurResponse user) {
        if (avatarImageView != null) {
    ...
    private void handleSave() {
        ...
        String dateStr = dateVal != null
                ? dateVal.format(DateTimeFormatter.ISO_LOCAL_DATE)
                : origDateStr;
        ...
        new Thread(() -> {
            try {
                ...
                if (pendingAvatarFile != null) {
                    UtilisateurResponse afterUp = ApiClient.uploadMyAvatar(pendingAvatarFile);
                    avatarForPut = afterUp.getAvatar();
                    pendingAvatarFile = null;
                }
                UtilisateurResponse updated = ApiClient.updateMe(nom, prenom, email,
                        tel.isEmpty() ? null : tel, dateStr, avatarForPut, pwdChange);
