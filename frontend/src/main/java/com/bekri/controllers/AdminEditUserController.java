...
public class AdminEditUserController implements Initializable {
    ...
    public void setUser(UtilisateurResponse user) {
        ...
        roleBadge.setText(capitalize(user.getRole()));
        roleBadge.getStyleClass().addAll("role-badge-small", "badge-" + user.getRole());

        telCard.setText(user.getTelephone() != null ? user.getTelephone() : "—");
        dateCard.setText(user.getCreatedAt() != null ? user.getCreatedAt().substring(0, 10) : "—");
        ...
        loadedDateIso = null;
        if (user.getDateNaissance() != null && user.getDateNaissance().length() >= 10) {
            loadedDateIso = user.getDateNaissance().substring(0, 10);
        } else {
            loadedDateIso = "2000-01-01";
        }

        roleCombo.setValue(displayRole(user.getRole()));
        statutCombo.setValue(displayStatut(user.getStatut()));
    ...
    private void handleSave() {
        ...
        new Thread(() -> {
            try {
                ...
                if (!newRole.equalsIgnoreCase(currentUser.getRole())) {
                    ApiClient.patchRole(currentUser.getId(), newRole);
                }
                String currentStatut = currentUser.getStatut() != null ? currentUser.getStatut() : "actif";
                if (!newStatut.equalsIgnoreCase(currentStatut)) {
                    ApiClient.patchStatut(currentUser.getId(), newStatut);
                }

                UtilisateurResponse updated = ApiClient.getUtilisateurById(currentUser.getId());
                ...
                Platform.runLater(() -> {
                    ...
                    setUser(updated);
                    roleBadge.getStyleClass().removeIf(c -> c.startsWith("badge-"));
                    roleBadge.setText(capitalize(updated.getRole()));
                    roleBadge.getStyleClass().addAll("role-badge-small", "badge-" + updated.getRole());
